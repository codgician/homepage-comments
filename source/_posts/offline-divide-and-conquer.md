...
离线分治算法的主要思想就是把分治思想用在操作集上，而其在时间复杂度上的代价仅仅是多乘上一个 $\mathcal{O}(\log{n})$。 本文将介绍两种离线分治算法：
...
假设题目给定了由 $n$ 个询问组成的一个询问集合 $Q$，并且我们知道所有出现过的数值的区间为 $[l, r]$。二分过程中，我们记 $mid = \frac{1}{2}(l+r)$。在每一层递归中，我们首先对于 $Q$ 内每一个询问，求得该询问区间中数值在 $[l, mid]$ 范围内的值的个数 $cnt$。接下来，我们以此将 $Q$ 中的询问分为两类：$cnt \ge k$ 的以及 $cnt < k$ 的。我们用 $Q	_1$ 和 $Q_2$ 来表示这两类询问。
