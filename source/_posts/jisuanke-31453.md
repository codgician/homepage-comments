---
title: 计蒜客 31453 - Hard to prepare
...
这里提供我比赛时超麻烦的思路…… 大神魔改错排公式的做法本垃圾没看懂…… 看懂后再补上…… 嘤嘤嘤。 
...
我们考虑已有 $i - 1$ 名宾客围成一圈的情况，并尝试在第 $i - 1$ 名宾客和第 $1$ 名宾客间插入第 $i$ 名宾客。我们很快意识到 $a_1$ 与 $a_{i - 1}$ 是否相等影响着我们对 $a_i$ 选择的余地：
...
- 对于 $dp[i - 1][0]$ 描述的情况，由于 $a_{i - 1} + a_1 \neq 2^k - 1$，所以可以直接在 $i$ 处插入 $a_1$ 使得首尾相等；
- 对于 $dp[i - 1][1]$ 描述的情况，显然可以直接插入 $a_1$；
